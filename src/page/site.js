...
/* preventDefault is the whole fix, not a tidy-up. openMgr() moves focus to the close
   button, and Enter's default action is dispatched AFTER this listener runs -- against
   whatever holds focus by then. So the card opened and the browser immediately clicked
   the ✕, and a keyboard user saw a flash and nothing else. Space is handled too: these
   spans are role="button", and a button is meant to answer to both keys. */
...
/* Left column leads with the roster; everything about scoring lives together on the
   right. The assert is the point of writing it this way: add a section to RULES and
...
(function(){const placed=RULE_COLS.flat(), all=RULES.map(r=>r[0]);
  ...
  if(missing.length||unknown.length)
    console.error('rules columns out of sync',{missing,unknown});})();
const rulesSection=h=>{const r=RULES.find(x=>x[0]===h); if(!r)return '';
  return `<div><h4>${r[0]}</h4><ul>`+r[1].map(([t,d])=>
    `<li>${t}${d?` <span class="dim">(default ${d})</span>`:''}</li>`).join('')+'</ul></div>';};
const rulesHTML=()=>RULE_COLS.map(names=>
  `<div class="rules-col">${names.map(rulesSection).join('')}</div>`).join('');
...
/* The record book follows the manager filter, so it can answer "what are the records
   among just these guys". It did once before and that went wrong: the filter defaults
   to the active ten, and eleven of these twenty-one tables quietly named the wrong
   record holder -- Alpert's 2021 as the highest-scoring season, when it is Watson's
   2018. So every table is ranked twice. The top five come from the managers selected;
   the #1 comes from all twenty, and if that holder is being filtered out he is named
   under the table. A narrowed view can never pass itself off as the league record.
   The shareable record cards (bigRecords) always use the full league. */
function drawRecords(){
  ...
  const q=M.filter(m=>m.seasons>=2), MV=ROWS.filter(r=>r.mv!=null);
  const f2=v=>v.toFixed(2), f1=v=>v.toFixed(1), pc=v=>(100*v).toFixed(1)+'%';
  const dif=r=>(r.pf-r.pa)/r.g, pts=r=>r.pf.toFixed(2)+' <span class="dim">/'+r.g+'g</span>';
  /* [title, every candidate in the league, rank by, 1 = highest first, show, row kind] */
  const B=[['SINGLE SEASON'],
   ['Highest PPG',ROWS,r=>r.ppg,1,r=>f2(r.ppg),'s'],
   ['Lowest PPG',ROWS,r=>r.ppg,0,r=>f2(r.ppg),'s'],
   ['Highest power index',ROWS,r=>r.pi,1,r=>f1(r.pi),'s'],
   ['Lowest power index',ROWS,r=>r.pi,0,r=>f1(r.pi),'s'],
   ['Best Z-score',ROWS,r=>r.z,1,r=>f2(r.z),'s'],
   ['Best differential per game',ROWS,dif,1,r=>f2(dif(r)),'s'],
   ['Worst differential per game',ROWS,dif,0,r=>f2(dif(r)),'s'],
   ['Luckiest season',ROWS,r=>r.luck,1,r=>f2(r.luck),'s'],
   ['Unluckiest season',ROWS,r=>r.luck,0,r=>f2(r.luck),'s'],
   ['Most roster moves',MV,r=>r.mv,1,r=>r.mv,'s'],
   ['COUNTING RECORDS &mdash; totals, not comparable across seasons'],
   ['Most points in a season',ROWS,r=>r.pf,1,pts,'s'],
   ['Fewest points in a season',ROWS,r=>r.pf,0,pts,'s'],
   ['CAREER RATES &mdash; 2+ seasons'],
   ['Best win %',q,m=>m.winpct,1,m=>pc(m.winpct),'c'],
   ['Worst win %',q,m=>m.winpct,0,m=>pc(m.winpct),'c'],
   ['Best average finish',q,m=>m.avgPlace,0,m=>f2(m.avgPlace),'c'],
   ['Best power index',q,m=>m.cpi,1,m=>f1(m.cpi),'c'],
   ['Worst power index',q,m=>m.cpi,0,m=>f1(m.cpi),'c'],
   ['Luckiest career',q,m=>m.luck,1,m=>f2(m.luck),'c'],
   ['Unluckiest career',q,m=>m.luck,0,m=>f2(m.luck),'c'],
   ['CAREER TOTALS &mdash; short careers included'],
   ['Most career points',M,m=>m.pf,1,m=>m.pf.toFixed(0),'c'],
   ['Most playoff wins',M,m=>m.poW,1,m=>m.poW,'c']];
  const who=(x,kind)=>kind==='s'?x.mgr:x.name;
  const narrowed=SEL.size<ALLNAMES.length;
  let h=narrowed
    ?`<p class="dim" style="margin:4px 0 0;font-size:13px">${SEL.size?`Filtered to <b>${SEL.size} of ${ALLNAMES.length}</b> managers.`:'<b>No managers selected</b> &mdash; pick some with the filter at the top.'} League records held by anyone outside the selection are named under each table.</p>`
    :'';
  let open=false;
  B.forEach(([t,all,k,d,val,kind])=>{
    if(!all){if(open)h+='</div>';
    ...
    const shown=S(all.filter(x=>vis(who(x,kind))),k,d);
    const lead=S(all,k,d)[0];
    const row=x=>kind==='s'?sr(x,val(x)):cr(x,val(x));
    const note=lead&&!vis(who(lead,kind))
      ?`<p class="dim" style="margin:0;padding:8px 12px 10px;font-size:12px;border-top:1px solid var(--rule-2)">League record, outside your selection: ${mlink(who(lead,kind))} &middot; ${kind==='s'?esc(lead.team)+' &middot; '+lead.y:lead.seasons+' seasons'} &middot; <b style="color:var(--ink)">${val(lead)}</b></p>`
      :'';
    ...
    h+=`<div class="card"><div class="card-h"><h3>${t}</h3>${gk?`<span class="gl" data-gl="${gk}" tabindex="0">?</span>`:''}</div><div class="scroll"><table>${
      shown.map((r,i)=>`<tr><td class="rk">${i+1}</td>${row(r)}</tr>`).join('')}</table></div>${note}</div>`;});
...
(function(){
  ...
  b.addEventListener('click',async e=>{
    e.stopPropagation();
    /* Reading WR.paused, awaiting, then restoring it looks like a race, and a linter
       flags it as one: a second click would capture was=true and leave the story stuck
       paused for good. It cannot happen. shareAny() sets btn.disabled=true synchronously
       before its first await, and the browser does not dispatch click on a disabled
       button, so no second handler ever starts. Verified by double-clicking it with a
       deliberately slow share. If shareAny ever stops disabling the button, this becomes
       a real bug -- keep the two together. */
