  ...
  <section id="weekly">
    ...
    <div class="card">
      <div class="card-h"><h3>Career races &mdash; one manager, every season</h3>
        <span class="sub">Every season with a game log &middot; not tied to the season buttons above</span></div>
...
function drawCareerRace(){
  ...
  const MID=cssv('--mid'), BR=cssv('--brass');
  const col=i=>S.length<2?BR:mix(MID,BR,0.18+0.82*i/(S.length-1));
  ...
  const lines=S.map((s,i)=>{
    ...
    return {y:s.y,team:s.team,K:s.K,R,d,c:col(i),seedRank:endRank,fin:fp,moved:endRank-fp,
            fx:FINX,fy:ys(fp),
  ...
  const paths=lines.map(l=>
    `<path class="crl" d="${l.d}" fill="none" stroke="${l.c}" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round" data-cy="${l.y}"/>`+
    `<path class="crl" d="${l.tail}" fill="none" stroke="${l.c}" stroke-width="2.2" stroke-dasharray="4 3" stroke-linecap="round" data-cy="${l.y}"/>`).join('');
  ...
  lines.forEach(l=>{l.R.forEach((p,k)=>{
      dots+=`<circle class="crd" cx="${xs(k)}" cy="${ys(p.rank)}" r="4.2" fill="var(--surface)" stroke="${l.c}" stroke-width="2" data-cy="${l.y}" data-i="${k}" style="cursor:pointer"/>`;});
    dots+=`<circle class="crd" cx="${l.fx}" cy="${l.fy}" r="5.6" fill="var(--surface)" stroke="${l.c}" stroke-width="2.4" data-cy="${l.y}" data-fin="1" style="cursor:pointer"/>`;});
  /* labels: keep one line-height apart, in finishing order */
  const ordered=lines.slice().sort((a,b)=>a.fy-b.fy);
  const LY=[]; ordered.forEach((l,i)=>{let y=l.fy+3.5; if(i&&y-LY[i-1]<14)y=LY[i-1]+14; LY.push(y); l.ly=y;});
  const labs=ordered.map(l=>{
    const nm=l.team.length>17?l.team.slice(0,16)+'…':l.team;
    return `<text class="crt" x="${W-P.r+13}" y="${l.ly}" font-size="11.5" font-family="IBM Plex Sans,sans-serif" data-cy="${l.y}" style="cursor:pointer">`+
  ...
  let medals='';
  ordered.forEach(l=>{const md=MEDAL[l.fin]; if(!md)return;
    const mx=W-P.r+2, my=l.ly-3.6;
    medals+=`<g class="crmed" style="pointer-events:none"><circle cx="${mx}" cy="${my}" r="6.4" fill="${md.f}" stroke="${md.s}" stroke-width="1.5"/>`+
      `<text x="${mx}" y="${my+3.1}" text-anchor="middle" font-size="8.4" font-weight="700" fill="${md.i}" font-family="IBM Plex Mono,monospace">${l.fin}</text></g>`;});

  host.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto" role="img" aria-label="Season-by-season league position for ${esc(CRMGR)}">${grid}${paths}${dots}${labs}${medals}${hits}</svg>`;
  ...
  function paint(hov){
    ...
    $$('#crace .crl').forEach(p=>{const sel=on.has(p.dataset.cy);
      p.setAttribute('opacity',!any||sel?1:.13);
      p.setAttribute('stroke-width',sel?3.4:2.2);});
