...
/* Two fixed columns, not auto-flow: the roster has to sit top-left and every scoring
...
/* the two-manager compare table: th,td are nowrap globally, so two long manager names
   plus the label column came to 331px and dragged the whole page sideways on a 320px
   screen. Let the names wrap rather than adding another horizontal scroller. */
#cmpOut th,#cmpOut td{white-space:normal;overflow-wrap:break-word}
@media(max-width:345px){#cmpOut th,#cmpOut td{padding:6px 5px}}
/* the record-to-share picker sizes itself to its longest option label (~300px) and was
   the next thing sticking off a 320px screen once the table above stopped doing it.
   The 8px is its own inline margin-left. */
#recPick{max-width:calc(100% - 8px)}
...
/* The record book deliberately ignores the manager filter. Its own opening paragraph
   promises that single-season records include everyone, and the career totals heading
   says the same -- but this used to filter like every other section, and the filter
   defaults to the active ten. Eleven of these twenty-one tables named the wrong record
   holder unless the reader happened to press "All 20" first: the real highest-scoring
   season is Watson's 2018, not Alpert's 2021. Only the 2+ seasons cut below is real. */
function drawRecords(){
  /* ROWS and M are the unfiltered globals on purpose -- see above. The shareable record
     cards (bigRecords) always read those, so the table and the card now agree. */
